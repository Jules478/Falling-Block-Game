...
void	export_name_to_score(t_tetris *tetris, char *name)
{
	...
	snprintf(full, sizeof(full), "%s: %s", name, tetris->score_str);
	free(tetris->score_str);
	tetris->score_str = strdup(full);
...
void	game_over_screen(t_tetris *tetris)
{
	...
	char score[20];
	int len = ft_strlen(tetris->score_str);
	int	i = -1;
	static char	name[4];
	int	c;
	...
	snprintf(score, sizeof(score), "Your Score: %s", tetris->score_str);
